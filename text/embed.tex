...
\colorbox[rgb]{0,1,0}{Zdůvodnit proč tu nejsou všechny modely daných řad a někteří výrobci.}

\colorbox[rgb]{0,1,0}{Upřesnit vazbu na UniPi, RPi a UART modul.}
...
\label{KapArduino}
	...
	Výrobce těchto desek vytvořil vývojové prostředí shodné pro všechny produkty Ardiuno. To se nazvývá Arduino IDE, je dostupné zdarma na webu výrobce a podporuje jazyk Wiring \cite{AboutWiring2011}, což je upravená verze jazyka C. Prostředí zároveň obsahuje i Serial Monitor, který slouží k oboustranné sériové komunikaci mezi Arduinem a PC. Alternativou ještě může být prostředí Processing \cite{AboutProcessing2015} využívající stejnojmenný jazyk, umožňující vytváření grafických multiplatformních aplikací.
...
\end{figure}
		...
		Lilypad Arduino je postaveno na ATmega168V (energeticky úsporná verze ATmega168) nebo ATmega328V. Je určena wearables projekty, zejména pro implementaci do textilií, kdy jsou spoje tvořeny vodivou nití. Není však pratelná. Existuje více variant této desky \cite{ArduinoLilipad}.	
			\begin{figure}[!h]
 \begin{center}
    \includegraphics[scale=0.1]{obrazky/emded_arduino_lilipad}
  \end{center}
  \caption{LilyPad Arduino [docasne prevzeti]}
\end{figure}
	...
	\begin{figure}[!h]
  \begin{center}
    \includegraphics[scale=0.05]{obrazky/emded_arduino_yun}
  \end{center}
  \caption{Arduino YUN [docasne prevzeti]}
\end{figure}
...
\end{figure}
	...
	%				\begin{figure}[!h]
  %\begin{center}
   % \includegraphics[scale=0.4]{obrazky/embed_raspi_2}
  %\end{center}
  %\caption{RaspberryPi 2 [docasne prevzeti]}
%\end{figure}
...
\subsection{RaspberryPi 3}
		...
		%		\begin{figure}[!h]
  %\begin{center}
   % \includegraphics[scale=0.2]{obrazky/embed_raspi_3}
  %\end{center}
  %\caption{RaspberryPi 3 [docasne prevzeti]}
%\end{figure}
...
\subsection{RaspberryPi Zero}
		...
		%\begin{figure}[!h]
  %\begin{center}
   % \includegraphics[scale=0.2]{obrazky/embed_raspi_zero}
  %\end{center}
  %\caption{RaspberryPi Zero [docasne prevzeti]}
%\end{figure}
