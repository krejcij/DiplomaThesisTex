...
\newpage
Obecná struktura~\cite{ModulIQRF}~paketu příkazu je	\texttt{>[CC][RW][DATA][CR],} kde CC je jednobajtový kód daného příkazu, RW je jednobajtový příznak zápisu (\textbf{:}) či čtení dat (\textbf{?}) dat, DATA jsou zapisovaná data, pokud se jedná o zápis a CR je znak ukončení.
